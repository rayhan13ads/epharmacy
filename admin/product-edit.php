...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['p_name'])) {
        $valid = 0;
        $error_message .= "Medicines name can not be empty<br>";
    }
    ...
    if($valid == 1) {
        ...
        if(isset($_POST['size'])) {
        	...
        	$statement = $pdo->prepare("DELETE FROM tbl_product_unit WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_unit (unit_id,p_id) VALUES (?,?)");
		...
		if(isset($_POST['color'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_power WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_power (power_id,p_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_power WHERE p_id=?");
...
$statement = $pdo->prepare("SELECT * FROM tbl_product_unit WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
	$unit_id[] = $row['unit_id'];
...
$statement = $pdo->prepare("SELECT * FROM tbl_product_power WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Medicines Name <span>*</span></label>
							<div class="col-sm-4">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select Unit</label>
							<div class="col-sm-4">
								<select name="size[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_unit ORDER BY unit_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										if(isset($unit_id)) {
											if(in_array($row['unit_id'],$unit_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
										...
										?>
										<option value="<?php echo $row['unit_id']; ?>" <?php echo $is_select; ?>><?php echo $row['unit_name']; ?></option>
										<?php
									}
									?>
								</select>
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select Power</label>
							<div class="col-sm-4">
								<select name="color[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_power ORDER BY power_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										if(isset($power_id)) {
											if(in_array($row['power_id'],$power_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
										...
										?>
										<option value="<?php echo $row['power_id']; ?>" <?php echo $is_select; ?>><?php echo $row['power_name']; ?></option>
										<?php
									}
									?>
								</select>
