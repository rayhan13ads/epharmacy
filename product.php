...
if(isset($_POST['form_add_to_cart'])) {
    ...
    if(isset($_SESSION['cart_p_id']))
    {
        ...
        if(!isset($_POST['power_id'])) {
            $power_id = 0;
        } else {
            $power_id = $_POST['power_id'];
        }
        for($i=1;$i<=count($arr_cart_p_id);$i++) {
            if( ($arr_cart_p_id[$i]==$_REQUEST['id']) && ($arr_cart_size_id[$i]==$unit_id) && ($arr_cart_color_id[$i]==$power_id) ) {
                $added = 1;
                break;
            }
        }
        if($added == 1) {
           $error_message1 = 'This product is already added to the shopping cart.';
            ...
            if(isset($_POST['unit_id'])) {
                ...
                $statement = $pdo->prepare("SELECT * FROM tbl_unit WHERE unit_id=?");
            ...
            if(isset($_POST['power_id'])) {
                ...
                $statement->execute(array($power_id));
                $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
                foreach ($result as $row) {
            ...
            $_SESSION['cart_size_id'][$new_key] = $unit_id;
            $_SESSION['cart_size_name'][$new_key] = $unit_name;
            $_SESSION['cart_color_id'][$new_key] = $power_id;
            $_SESSION['cart_color_name'][$new_key] = $power_name;
    ...
    else
    {
        ...
        $_SESSION['cart_size_id'][1] = $unit_id;
        $_SESSION['cart_size_name'][1] = $unit_name;
        $_SESSION['cart_color_id'][1] = $power_id;
        $_SESSION['cart_color_name'][1] = $power_name;
                            ...
                            <div class="p-quantity">
                                <div class="row">
                                    ...
                                    <div class="col-md-12 mb_20">
                                        ...
                                        <select name="unit_id" class="form-control select2" style="width:auto;">
                                            <?php
                                            $statement = $pdo->prepare("SELECT * FROM tbl_unit");
                                            $statement->execute();
                                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($result as $row) {
                                                if(in_array($row['unit_id'],$size)) {
                                                    ?>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select name="power_id" class="form-control select2" style="width:auto;">
                                            <?php
                                            $statement = $pdo->prepare("SELECT * FROM tbl_power");
                                            $statement->execute();
                                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($result as $row) {
                                                if(in_array($row['power_id'],$color)) {
                                                    ?>
                                                    <option value="<?php echo $row['power_id']; ?>"><?php echo $row['power_name']; ?></option>
                                                    <?php
                                                }
                                            }
                                            ?>
